...
function print_restaurant_typeahead($restaurant_data)
{
  global $APP_ROOT_URL;
  $js_id_list = '';
  $js_name_list = '';
  $id_array = array();
  $name_array = array();
  foreach ($restaurant_data as $rest_id => $data)
  {
    $id_array[] = $rest_id;
    $name_array[] = addslashes($data['name']);
  }
  $js_id_list = "['" . implode("','", $id_array) . "']";
  $js_name_list = "['" . implode("','", $name_array) . "']";
?>
<script><!--
var autocomplete_ids = <?print $js_id_list?>;
var autocomplete = <?print $js_name_list?>;
var doc_location = '<?print $APP_ROOT_URL?>';
//['Facebook','foo','bar','FBJS','Safari','Firefox','Internet Explorer','Opera','Mario','Yoshi'];
//--></script>

<div>
  <input id="typeahead" class="inputtext th_placeholder" value="Type something here..." onfocus="new typeahead(document.getElementById('typeahead'), autocomplete, autocomplete_ids, doc_location)" /><br />
</div>
...
typeahead.prototype.onkeydown = function(event) {
  switch (event.keyCode) {
    case 9: // tab
    case 13: // enter
      document.setLocation(this.doc_location + "/index.php?action=view&restaurant="+ this.result_ids[this.selectedindex]);
...
typeahead.prototype.update_results = function() {
  ...
  for (var i = 0; i < this.options.length; i++) {
    ...
    if (prefix == val) {
      ...
      result_ids.push(this.option_ids[i]);
