...
function get_prints($user) {
  $conn = get_db_conn();
  $res = mysql_query('SELECT `from`, `to`, `time` FROM footprints WHERE `to`=' . $user . ' ORDER BY `time` DESC', $conn) or die(mysql_error());
  $prints = array();
...
function do_step($from, $to) {
  ...
  try {
    ...
    // Publish feed story
    ## this is the old one, so commented
    ...
    ## feed publish story - new
    /**
   * Sends a notification to the specified users.
   *
   * @return A comma separated list of successful recipients
   */
   $template_bundle_id = '104049820577';
   $template_data = array('actor' => $from);
   $facebook->api_client->feed_publishUserAction($template_bundle_id, $template_data);
   ###########################
